...
START_TEST(add_matrix) {
  ...
  // все данные корректны, квадратные матрицы, сложение с 0й матрицей
  {
    int row = 3;
    int col = 3;
    matrix_t A = {NULL, row, col};
  ...
  // все данные корректны, квадратные матрицы, сложение с противоположной
  // матрицей
  {
    int row = 3;
    int col = 3;
    ...
    double mas_a[3][3] = {{2.7, 5.7, 7.7}, {6.7, 3.7, 4.7}, {5.7, -2.7, -3.7}};
    double mas_b[3][3] = {
        {-2.7, -5.7, -7.7}, {-6.7, -3.7, -4.7}, {-5.7, 2.7, 3.7}};
    double mas_ch[3][3] = {0};
  ...
  // Подаются матрицы разных размеров
  {
    ...
    ck_assert_int_eq(s21_sum_matrix(&A, &B, &result), OPERATION_ERROR);
    s21_remove_matrix(&A);
    s21_remove_matrix(&B);
  }
...
Suite* suite_s21_add_matrix() {
  Suite* s = suite_create("add_matrix");
  TCase* tc = tcase_create("add_matrix_tc");

  tcase_add_test(tc, add_matrix);
